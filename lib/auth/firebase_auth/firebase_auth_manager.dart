    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Pasó demasiado tiempo desde el último inicio de sesión. Inicie sesión nuevamente antes de borrar su cuenta.')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Pasó demasiado tiempo desde el último inicio de sesión. Inicie sesión nuevamente antes de actualizar su correo electrónico.')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Error al tratar de iniciar sesion, confirme que esta conectado a wi-fi')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Email para restablecer contraseña enviado')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : MenteSanaFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' =>
          'Este email ya esta en uso, favor de ingresar uno nuevo',
        'INVALID_LOGIN_CREDENTIALS' =>
          'Email o contraseña incorrectos, verifiquelos para acceder',
        _ =>
          'Error al tratar de iniciar sesion, confirme que esta conectado a wi-fi',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
